...
export async function searchAccommodation(params: {
  ...
  const internalCatalogContext = params.internalCatalog && params.internalCatalog.length > 0
    ? `\n\n⭐ CATALOGUE INTERNE (LUNA TRAVEL):\nVoici des établissements partenaires de notre agence. PRIORISE FORTEMENT ces hôtels locaux s'ils correspondent à la destination :\n${JSON.stringify(params.internalCatalog.filter(i => i.type === 'HOTEL'), null, 2)}`
    : '';
...
2. ⭐ Utilise le CATALOGUE INTERNE LUNA TRAVEL en priorité absolue s'il correspond à la ville.
3. Hôtels qui EXISTENT RÉELLEMENT dans chaque ville.
4. 💰 Prix RÉELS par nuit.
5. ⭐ Mélange de gammes (3★ à 5★) DANS CHAQUE ville.${realHotelsContext}${internalCatalogContext}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
` : '';
  ...
  const internalCatalogContext = params.internalCatalog && params.internalCatalog.length > 0
    ? `\n\n⭐ CATALOGUE INTERNE (LUNA TRAVEL):\nVoici des activités, transferts et restaurants exclusifs partenaires de l'agence. INTÈGRE obligatoirement ces prestataires précis dans l'itinéraire (en respectant la cohérence de leur ville d'emplacement) :\n${JSON.stringify(params.internalCatalog.filter(i => ['ACTIVITY', 'TRANSFER', 'OTHER', 'RESTAURANT', 'EXPERIENCE'].includes(i.type)), null, 2)}`
    : '';
...
2. ⭐ Intégrer les prestataires du CATALOGUE INTERNE fournis ci-dessous dans les jours d'itinéraire !
3. 📐 Exactement ${numDays} jours, pas plus pas moins
4. 🕐 Rythme réaliste: pas trop d'activités par jour
5. 🌟 Chaque jour a un "highlight" — le moment fort
6. 🍽️ Au moins 1 restaurant nommé par jour
7. 📌 Chaque jour a un champ "destination" = nom de la ville
${internalCatalogContext}
