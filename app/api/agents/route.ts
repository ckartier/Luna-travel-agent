...
export async function POST(request: Request) {
    ...
    try {
        ...
        // ── 1.5 Fetch Semantic Internal Catalog (Luna Travel's own inventory) ──
        let internalCatalog: any[] = [];
        try {
            // Get default tenant
            ...
            if (!tenantsSnap.empty) {
                ...
                // 1. Generate text embedding for the user's dream trip
                const queryText = `Voyage à ${dest} ambiance ${vibe || 'luxe'} pour ${pax} personnes. ${mustHaves || ''}`;
                const queryEmbedding = await generateMultimodalEmbedding({ text: queryText, dimension: 768 });
                
                if (queryEmbedding) {
                    // 2. Search Firestore Vector Database for top 10 closest catalog items
                    ...
                    internalCatalog = results.map(r => r.item);
                    console.log(`[Semantic Search] Found ${internalCatalog.length} internal matching items.`);
                }
            }
        } catch (e) {
            console.warn('[Semantic Search] Failed to fetch internal catalog:', e);
        }
