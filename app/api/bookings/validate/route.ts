...
export async function POST(request: Request) {
    ...
    // SECURITY: Use tenantId from auth token, NOT from request body (prevents IDOR)
    ...
    try {
        ...
        if (supplier?.phone) {
            ...
            if (token && phoneId) {
                ...
                const confirmMessage =
                    `✅ *Votre prestation est validée !* 🎉\n\n` +
                    `Bonjour ${supplierName},\n\n` +
                    `Votre prestation a été officiellement *confirmée et validée* ✨\n\n` +
                    `📋 *Récapitulatif Final :*\n` +
        ...
        let invoiceId = null;
        try {
            const { createInvoice } = await import('@/src/lib/firebase/crm');
            ...
            const invoiceData = {
                ...
                clientName: supplier?.name || 'Prestataire',
                supplierId: booking.supplierId,
                supplierName: supplier?.name || 'Prestataire',
                issueDate: new Date().toISOString().slice(0, 10),
                dueDate: new Date(bookingDate.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
                items: [{
                    description: `${booking.prestationName || 'Prestation'} — ${supplier?.name || 'Prestataire'} (${booking.date})`,
                    quantity: booking.numberOfGuests || 1,
                    unitPrice: booking.rate || 0,
                    total: (booking.rate || 0) * (booking.numberOfGuests || 1),
                    taxRate: 0,
                }],
                subtotal: (booking.rate || 0) * (booking.numberOfGuests || 1),
                taxTotal: 0,
                totalAmount: (booking.rate || 0) * (booking.numberOfGuests || 1) + (booking.extraFees || 0),
                currency: 'EUR',
                amountPaid: 0,
                status: 'DRAFT' as const,
                notes: `Facture auto-générée — Prestation confirmée.\nBooking: ${bookingId}\nDate: ${booking.date} ${booking.startTime || ''}`,
            };
            invoiceId = await createInvoice(tenantId, invoiceData);

            // Link invoice ref to booking notes
            await bookingRef.update({
                notes: `${booking.notes || ''}\n📄 Facture: ${invoiceData.invoiceNumber}`.trim(),
            });
            console.log(`[Luna Validate] 📄 Auto-invoice ${invoiceData.invoiceNumber} created for ${supplier?.name}`);
        } catch (invErr: any) {
            console.error('[Luna Validate] Invoice creation error:', invErr.message);
        }
