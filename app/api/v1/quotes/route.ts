...
export async function GET(req: NextRequest) {
    ...
    const status = url.searchParams.get('status'); // DRAFT | SENT | ACCEPTED | REJECTED

    try {
        const tenantRef = adminDb.collection('tenants').doc(auth.tenantId);
        const snap = await tenantRef.collection('quotes').orderBy('createdAt', 'desc').limit(limit).offset(offset).get();

        let quotes = snap.docs.map(d => {
            const data = d.data();
            return {
                id: d.id,
        ...
        return apiSuccess({ quotes, total: quotes.length, limit, offset });
    } catch (error: any) {
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const { clientName, items, currency, validDays } = body;
        ...
        const countSnap = await tenantRef.collection('quotes').count().get();
        const quoteNumber = `DEV-${String(countSnap.data().count + 1).padStart(4, '0')}`;

        const docRef = await tenantRef.collection('quotes').add({
            quoteNumber,
            clientName,
        ...
        return apiSuccess({ id: docRef.id, quoteNumber, totalAmount, currency: currency || 'EUR' }, 201);
    } catch (error: any) {
        return apiError(error.message, 500);
    }
}
