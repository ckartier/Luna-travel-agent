...
export default function SettingsPage() {
  ...
  // Fetch Google Cloud service status & Billing Usage
  useEffect(() => {
    if (!user) return;
    (async () => {
      try {
        ...
        const [googleRes, usageRes] = await Promise.all([
          fetchWithAuth('/api/crm/google-status', { headers }),
          fetchWithAuth('/api/crm/billing-summary', { headers })
        ]);
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-luna-cream/60 rounded-xl px-4 py-3 border border-luna-warm-gray/8">
                      <div className="flex items-center gap-1.5 text-luna-text-muted mb-1">
                        <Clock size={12} />
                        <span className="text-[12px] uppercase tracking-wider font-normal"><T>Membre</T></span>
                      </div>
                    ...
                    <div className="bg-luna-cream/60 rounded-xl px-4 py-3 border border-luna-warm-gray/8">
                      <div className="flex items-center gap-1.5 text-luna-text-muted mb-1">
                        <Sparkles size={12} />
                        <span className="text-[12px] uppercase tracking-wider font-normal"><T>Statut</T></span>
                      </div>
                ...
                <div className="px-5 pb-3">
                  <div className="bg-gradient-to-br from-violet-50/80 to-sky-50/80 rounded-xl p-4 border border-violet-100/40">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[12px] uppercase tracking-[0.15em] font-normal text-violet-600"><T>Abonnement</T></span>
                    </div>
                    {isActive ? (
                      <>
                        <p className="text-sm font-normal text-luna-charcoal mb-3">{planName || 'Actif'}</p>
                        
                        {/* Usage Gauges */}
                        {!usageLoading && usageSummary && (
                          <div className="space-y-3 mt-1 mb-4 border-t border-violet-100/50 pt-3">
                            {/* Contacts */}
                            <div>
                                <div className="flex justify-between text-[11px] mb-1">
                                    <span className="text-luna-text-muted">Contacts</span>
                                    <span className="font-medium text-luna-charcoal">
                                        {usageSummary.contacts.current} / {usageSummary.contacts.limit === -1 ? '∞' : usageSummary.contacts.limit}
                                    </span>
                                </div>
                                <div className="h-1.5 w-full bg-violet-100 rounded-full overflow-hidden">
                                    <div 
                                        className={`h-full rounded-full transition-all ${usageSummary.contacts.limit !== -1 && usageSummary.contacts.current >= usageSummary.contacts.limit ? 'bg-red-400' : 'bg-violet-400'}`}
                                        style={{ width: `${usageSummary.contacts.limit === -1 ? 10 : Math.min(100, (usageSummary.contacts.current / usageSummary.contacts.limit) * 100)}%` }}
                                    />
                                </div>
                            </div>
                            ...
                            <div>
                                <div className="flex justify-between text-[11px] mb-1">
                                    <span className="text-luna-text-muted">Voyages / mois</span>
                                    <span className="font-medium text-luna-charcoal">
                                        {usageSummary.tripsThisMonth.current} / {usageSummary.tripsThisMonth.limit === -1 ? '∞' : usageSummary.tripsThisMonth.limit}
                                    </span>
                                </div>
                                <div className="h-1.5 w-full bg-sky-100 rounded-full overflow-hidden">
                                    <div 
                                        className={`h-full rounded-full transition-all ${usageSummary.tripsThisMonth.limit !== -1 && usageSummary.tripsThisMonth.current >= usageSummary.tripsThisMonth.limit ? 'bg-red-400' : 'bg-sky-400'}`}
                                        style={{ width: `${usageSummary.tripsThisMonth.limit === -1 ? 10 : Math.min(100, (usageSummary.tripsThisMonth.current / usageSummary.tripsThisMonth.limit) * 100)}%` }}
                                    />
                                </div>
                            </div>
                            ...
                            <div>
                                <div className="flex justify-between text-[11px] mb-1">
                                    <span className="text-luna-text-muted">Requêtes IA / jour</span>
                                    <span className="font-medium text-luna-charcoal">
                                        {usageSummary.aiToday.current} / {usageSummary.aiToday.limit === -1 ? '∞' : usageSummary.aiToday.limit}
                                    </span>
                                </div>
                                <div className="h-1.5 w-full bg-amber-100 rounded-full overflow-hidden">
                                    <div 
                                        className={`h-full rounded-full transition-all ${usageSummary.aiToday.limit !== -1 && usageSummary.aiToday.current >= usageSummary.aiToday.limit ? 'bg-red-400' : 'bg-amber-400'}`}
                                        style={{ width: `${usageSummary.aiToday.limit === -1 ? 10 : Math.min(100, (usageSummary.aiToday.current / usageSummary.aiToday.limit) * 100)}%` }}
                                    />
                                </div>
                            </div>
                          </div>
                      ...
                      <p className="text-xs text-luna-text-muted"><T>Aucun abonnement actif</T></p>
                    )}

                    {/* ── Plan Switcher (dev/test) ── */}
                    <div className="mt-3 pt-3 border-t border-violet-100/50">
                      <p className="text-[12px] uppercase tracking-wider text-violet-400 font-normal mb-2"><T>Changer de plan</T></p>
            ...
            >
              ...
              <div className="bg-white/85 backdrop-blur-2xl rounded-2xl border border-luna-warm-gray/12 shadow-sm p-6 md:p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 className="text-base font-normal text-luna-charcoal tracking-tight">Google Cloud & IA</h3>
                    <p className="text-[12px] text-luna-text-muted mt-0.5"><T>Services Google installés et configurés sur ce CRM</T></p>
                  </div>
                </div>
                ...
                {googleLoading ? (
                  ...
                  <div className="space-y-3">
                    {googleServices.map(service => {
                      ...
                      const color = colorMap[service.id] || '#6B7280';

                      return (
                        <div
                          key={service.id}
                          className="flex items-center gap-4 p-4 rounded-xl border border-luna-warm-gray/10 bg-luna-cream/30 hover:bg-luna-cream/60 transition-all group"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <h4 className="text-sm font-medium text-luna-charcoal">{service.name}</h4>
                              {service.model && (
                                <span className="text-[10px] font-mono bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">{service.model}</span>
                              )}
                              {service.voice && (
                                <span className="text-[10px] font-mono bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">{service.voice}</span>
                              )}
                            </div>
                            <p className="text-[11px] text-luna-text-muted mt-0.5 truncate">{service.description}</p>
                          </div>
                          <div className="flex items-center gap-3 shrink-0">
                            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                              ...
                              <span className={`w-1.5 h-1.5 rounded-full ${service.configured ? 'bg-emerald-500' : 'bg-gray-300'}`} />
                              {service.configured ? 'Connecté' : 'Non configuré'}
                            </div>
                            <a
                              href={service.consoleUrl}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="w-8 h-8 rounded-lg border border-luna-warm-gray/15 flex items-center justify-center text-luna-text-muted hover:text-luna-charcoal hover:border-luna-charcoal/30 transition-all opacity-0 group-hover:opacity-100"
