...
import {
  ...
  MapPin, Palette, Target, ClipboardList, Rocket, AlertCircle, Globe, Compass
} from 'lucide-react';
import {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 min-h-[80vh]">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <h1 className="text-4xl font-light text-[#2E2E2E] tracking-tight">Super Agent Luna</h1>
        ...
        {messages.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8">
            ...
            <div className="flex flex-wrap gap-3 justify-center max-w-lg">
              {(mode === 'CLIENT' ? [
                { icon: <Plane size={14} className="text-sky-500" />, text: 'Voyage Bali 10 jours couple, 6000€' },
                { icon: <Globe size={14} className="text-teal-500" />, text: 'Séjour Maldives luxe famille' },
                { icon: <Compass size={14} className="text-indigo-500" />, text: 'Circuit Japon 2 semaines aventure' },
              ] : [
                { icon: <Hotel size={14} className="text-amber-500" />, text: 'Quels hôtels j\'ai en Thaïlande ?' },
                { icon: <Target size={14} className="text-rose-500" />, text: 'Propose des activités pour Bali' },
                { icon: <ClipboardList size={14} className="text-violet-500" />, text: 'Compose un package tout compris Grèce' },
              ]).map((s, i) => (
                ...
                {msg.proposals && msg.proposals.length > 0 && (
                  <div className="max-w-3xl mx-auto mt-3 ml-11">
                    <p className="text-xs text-luna-text-muted mb-2 uppercase tracking-widest">
                      <Target size={12} className="text-gray-400 inline mr-1" />Prestations proposées — Validez celles que vous souhaitez
                    </p>
                        ...
                        <motion.div key={proposal.catalogItem.id}
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-luna-text-muted mt-0.5 flex items-center gap-1"><MapPin size={10} /> {proposal.catalogItem.location} • {proposal.catalogItem.supplier}</p>
              ...
              <div key={p.catalogItem.id} className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-luna-text-muted flex items-center gap-1"><MapPin size={10} /> {p.catalogItem.location}</p>
                  </div>
      ...
      <AnimatePresence>
        {showPlanningModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-luna-charcoal/60 backdrop-blur-xl flex items-center justify-center p-4"
            onClick={() => setShowPlanningModal(false)}>
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20 }}
              className="bg-white rounded-[40px] w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl border border-luna-warm-gray/10"
              onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-5 border-b border-luna-warm-gray/10">
                <h3 className="font-serif text-lg font-normal text-luna-charcoal flex items-center gap-2"><Calendar size={18} /> Créer le voyage & pousser au pipeline</h3>
              ...
              <div className="p-5 space-y-4">
                {/* Client selector */}
                <div>
                  <label className="text-xs uppercase tracking-widest text-luna-text-muted block mb-1.5 flex items-center gap-1"><User size={12} /> Client</label>
                  ...
                  {selectedClient && (
                    <p className="text-xs text-emerald-600 mt-1 flex items-center gap-1"><CheckCircle2 size={12} /> {selectedClient.firstName} {selectedClient.lastName}</p>
                  )}
                </div>
                ...
                {/* Prestations summary */}
                <div>
                  <p className="text-xs uppercase tracking-widest text-luna-text-muted mb-2 flex items-center gap-1"><Palette size={12} /> Prestations ({selectedPrestations.length})</p>
