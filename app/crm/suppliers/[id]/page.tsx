...
import {
    ...
    CalendarDays, TrendingUp, Wallet, Check, AlertCircle, Info, CheckCircle2
} from 'lucide-react';
import {
...
const CATEGORIES: { value: SupplierCategory; label: string; emoji: string; color: string }[] = [
    { value: 'HÉBERGEMENT', label: 'Hébergement', emoji: '🏨', color: 'bg-indigo-50 text-indigo-600 border-indigo-200' },
    { value: 'RESTAURANT', label: 'Restaurant', emoji: '🍽️', color: 'bg-orange-50 text-orange-600 border-orange-200' },
    { value: 'ACTIVITÉ', label: 'Activité', emoji: '🎯', color: 'bg-emerald-50 text-emerald-600 border-emerald-200' },
    { value: 'CULTURE', label: 'Culture', emoji: '🏛️', color: 'bg-purple-50 text-purple-600 border-purple-200' },
    { value: 'TRANSPORT', label: 'Transport', emoji: '🚗', color: 'bg-sky-50 text-sky-600 border-sky-200' },
    { value: 'GUIDE', label: 'Guide', emoji: '🧭', color: 'bg-amber-50 text-amber-600 border-amber-200' },
    { value: 'AUTRE', label: 'Autre', emoji: '📦', color: 'bg-gray-50 text-gray-600 border-gray-200' },
];
...
export default function SupplierDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const handleDelete = async () => {
        if (!supplier?.id || !confirm('Supprimer ce prestataire ?')) return;
    ...
    return (
        <div className="max-w-[1200px] mx-auto pb-20 animate-in fade-in duration-500">
            ...
            <div className="flex justify-between items-center mb-8 px-4 sm:px-0">
                <button onClick={() => router.push('/crm/suppliers')} className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-luna-charcoal transition-all group">
                    <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Retour Listing
                </button>
                <div className="flex gap-4">
                    ...
                    {!editing && (
                        <button onClick={handleDelete} className="p-3 bg-white border border-gray-100 text-rose-300 hover:text-rose-500 rounded-2xl shadow-sm hover:border-rose-100 transition-all active:scale-95">
                            <Trash2 size={20} />
                        </button>
                    )}
                </div>
            ...
            {/* ── MAIN HEADER CARD ── */}
            <div className="bg-white rounded-[60px] border border-gray-50 p-10 md:p-14 shadow-2xl shadow-gray-200/50 mb-12 overflow-hidden relative">
                <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-gradient-to-br from-indigo-50/20 to-emerald-50/10 rounded-bl-full -z-10" />

                <div className="flex flex-col lg:flex-row gap-12 items-start">
                    {/* Visual Identity */}
                    <div className="relative group mx-auto lg:mx-0">
                        <div className="w-40 h-40 rounded-[50px] bg-gradient-to-br from-gray-50 to-white border border-gray-100/50 flex items-center justify-center text-6xl shadow-2xl group-hover:rotate-6 transition-all duration-700">
                            {cat.emoji}
                        </div>
                        {supplier.isFavorite && (
                            <div className="absolute -top-3 -right-3 w-12 h-12 bg-rose-500 text-white rounded-full flex items-center justify-center shadow-xl border-4 border-white">
                                <Heart size={20} fill="currentColor" />
                            </div>
                        )}
                        <div className="mt-6 flex justify-center gap-1">
                            {[1, 2, 3, 4, 5].map(n => (
                                <Star key={n} size={14} className={n <= (supplier.rating || 0) ? 'fill-amber-400 text-amber-400' : 'text-gray-100'} />
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex-1 space-y-6 text-center lg:text-left">
                        <div className="space-y-2">
                            <div className="flex items-center gap-4 flex-wrap justify-center lg:justify-start">
                                <h1 className="text-4xl md:text-5xl font-normal text-luna-charcoal tracking-tighter uppercase">{supplier.name}</h1>
                                <span className={`px-5 py-2 rounded-2xl text-[10px] font-bold uppercase tracking-widest border transition-colors ${cat.color}`}>
                                    {cat.label}
                                </span>
                            </div>
                            <p className="text-sm text-gray-400 font-sans flex items-center gap-2 justify-center lg:justify-start">
                                <MapPin size={16} className="text-emerald-500" /> {[supplier.city, supplier.country].filter(Boolean).join(', ')}
                            </p>
                        </div>

                        <div className="flex flex-wrap gap-3 justify-center lg:justify-start pt-4">
                            {supplier.isGuide && <span className="px-5 py-2 bg-amber-50 text-amber-600 border border-amber-100 rounded-2xl text-[9px] font-bold uppercase tracking-widest flex items-center gap-2"><Briefcase size={12} /> Guide Agent</span>}
                            {supplier.isChauffeur && <span className="px-5 py-2 bg-sky-50 text-sky-600 border border-sky-100 rounded-2xl text-[9px] font-bold uppercase tracking-widest flex items-center gap-2">🚗 Chauffeur Pro</span>}
                            {supplier.hasLicense && <span className="px-5 py-2 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-2xl text-[9px] font-bold uppercase tracking-widest flex items-center gap-2"><ShieldCheck size={12} /> Licence OK</span>}
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 max-w-lg mx-auto lg:mx-0">
                            {supplier.email && <div className="flex items-center gap-3 px-6 py-4 bg-gray-50/50 rounded-3xl text-sm text-gray-500 font-sans border border-gray-100/50"><Mail size={16} className="text-emerald-500" /> {supplier.email}</div>}
                            {supplier.phone && <div className="flex items-center gap-3 px-6 py-4 bg-gray-50/50 rounded-3xl text-sm text-gray-500 font-sans border border-gray-100/50"><Phone size={16} className="text-emerald-500" /> {supplier.phone}</div>}
                        </div>
                    </div>

                    {/* Financial Quick Summary */}
                    <div className="w-full lg:w-auto flex flex-col gap-4">
                        <div className="p-8 bg-luna-charcoal text-white rounded-[40px] text-center shadow-2xl shadow-gray-200">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-400 mb-2">À Payer (Prestations)</p>
                            <p className="text-4xl font-normal tracking-tighter">{totalToPayToSupplier.toLocaleString('fr-FR')} €</p>
                            <div className="mt-4 w-full h-[1px] bg-white/10" />
                            <p className="text-[10px] font-bold uppercase tracking-widest text-white/30 mt-4 mb-2">Marge Nette Associée</p>
                            <p className="text-2xl font-normal tracking-tighter text-emerald-400">+{netMargin.toLocaleString('fr-FR')} €</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {/* ── TABS NAVIGATION ── */}
            <div className="flex gap-4 mb-10 overflow-x-auto pb-6 px-4 no-scrollbar">
                {[
                    ...
                    <button
                        ...
                        className={`px-10 py-5 rounded-[28px] text-[10px] font-bold uppercase tracking-widest flex items-center gap-3 transition-all border whitespace-nowrap active:scale-95 ${activeTab === tab.id ? 'bg-luna-charcoal text-white border-luna-charcoal shadow-2xl shadow-gray-200' : 'bg-white text-gray-400 border-gray-50 hover:border-emerald-100 hover:text-luna-charcoal'}`}
                    >
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="grid grid-cols-1 lg:grid-cols-3 gap-8 px-4 sm:px-0">
                            ...
                            <section>
                                ...
                                {editing ? (
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        ...
                                        )) || <p className="text-sm text-gray-400 italic">Aucune langue enregistrée.</p>}
                                    </div>
                                )}
                            </section>
                        ...
                        <div className="bg-luna-charcoal rounded-[50px] p-12 text-white relative flex flex-col justify-between shadow-2xl overflow-hidden min-h-[400px]">
                            ...
                            <div className="space-y-8 relative z-10">
                                ...
                                <p className="text-xl font-normal leading-relaxed text-white/80 italic">
                        ...
                        <div className="grid grid-cols-1 gap-6">
                                ...
                                <div key={i} className="bg-white p-10 rounded-[50px] border border-gray-50 hover:border-emerald-100 hover:shadow-2xl hover:shadow-emerald-50/50 transition-all flex flex-col lg:flex-row justify-between items-center gap-10 group relative">
                                    ...
                                    <div className="flex items-center gap-12 w-full lg:w-auto justify-center lg:justify-end">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button className="hidden lg:flex w-10 h-10 rounded-xl border border-gray-100 items-center justify-center text-gray-200 hover:text-emerald-500 transition-all">
                            ...
                            {bookings.length === 0 && (
                                <div className="py-24 text-center bg-gray-50/50 rounded-[60px] border border-dashed border-gray-100">
                                    ...
                                    <p className="text-sm text-gray-400 font-sans">Aucune mission n'a été planifiée pour {supplier.name}.</p>
                                </div>
                            )}
                        </div>
                    </motion.div>
                        ...
                        <div className="bg-white rounded-[60px] border border-gray-50 p-12 md:p-16 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                                <div className="p-8 bg-gray-50/50 rounded-[32px] border border-gray-100">
                                    ...
                                    <p className="text-3xl font-normal text-luna-charcoal">{totalToReceiveFromClient > 0 ? ((netMargin / totalToReceiveFromClient) * 100).toFixed(1) : 0}%</p>
                                ...
                                <div className="p-8 bg-gray-50/50 rounded-[32px] border border-gray-100">
                                    ...
                                    <p className="text-3xl font-normal text-luna-charcoal">{bookings.length > 0 ? (totalToPayToSupplier / bookings.length).toFixed(0) : 0} €</p>
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            ...
                            {prestations.length === 0 && (
                                <div className="lg:col-span-3 py-24 text-center bg-gray-50/50 rounded-[60px] border border-dashed border-gray-100">
                                    ...
                                    <p className="text-sm text-gray-400 font-sans">Aucune prestation de votre catalogue n'est liée à ce fournisseur.</p>
                                </div>
                            )}
                        </div>
                    </motion.div>
                ...
                {showBookingModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ opacity: 0, y: 50, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 50, scale: 0.95 }} className="bg-white rounded-[60px] w-full max-w-xl relative z-10 shadow-2xl p-12 md:p-16 overflow-hidden">
                            <h2 className="text-3xl font-normal text-luna-charcoal mb-8 uppercase tracking-tighter">Affectation Mission</h2>
            ...
            <AnimatePresence>
                {editing && (
                    ...
                    >
                        <div className="bg-luna-charcoal text-white rounded-[40px] p-8 shadow-2xl flex items-center justify-between gap-10 border border-white/10 backdrop-blur-3xl">
                            <div className="hidden sm:block">
                                <p className="text-xs font-bold text-white/40 uppercase tracking-widest">Luna Executive Sync</p>
                                <p className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest">Drafting changes...</p>
                            </div>
                            <div className="flex gap-4 w-full sm:w-auto">
                                <button onClick={() => { setEditing(false); setEditData(supplier); }} className="flex-1 sm:flex-none px-8 py-4 bg-white/10 hover:bg-white/20 rounded-[20px] text-[10px] font-bold uppercase tracking-widest transition-all">Abandonner</button>
                                <button onClick={handleSave} disabled={saving} className="flex-[2] sm:flex-none px-10 py-4 bg-emerald-500 hover:bg-emerald-600 rounded-[20px] text-[10px] font-bold uppercase tracking-widest transition-all shadow-2xl shadow-emerald-500/30">
                                    {saving ? 'Synchronisation...' : 'Enregistrer la Fiche'}
                                </button>
                            </div>
                        </div>
                    </motion.div>
...
function ChevronRight({ size, className }: { size?: number, className?: string }) {
    return <ArrowLeft className={`rotate-180 ${className || ''}`} size={size} />;
}
