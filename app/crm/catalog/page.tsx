    ...
    <div className="min-h-screen">
      ...
      <div className="flex gap-3 mb-6">
        ...
        {['ALL', 'HOTEL', 'FLIGHT', 'ACTIVITY', 'TRANSFER', 'OTHER'].map(f => (
          <button key={f} onClick={() => setFilter(f)} className={`px-3.5 py-2.5 rounded-xl text-xs font-medium transition-all ${filter === f ? 'bg-luna-charcoal hover:bg-gray-800 text-white ' : 'bg-white/60 text-gray-500 hover:bg-white hover:shadow-sm'}`}>
            {f === 'ALL' ? 'Tout' : f}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(item => (
            <div key={item.id} className="bg-white/70 backdrop-blur-sm rounded-2xl p-5 hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] hover:-translate-y-0.5 transition-all group">
