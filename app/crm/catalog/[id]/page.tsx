...
import {
    ArrowLeft, Edit3, Save, X, Trash2, Heart, Star, Send, Copy, Check,
    Hotel, Plane, MapPin, Car, UtensilsCrossed, Camera, DollarSign,
...
const SUPPLIER_CATS: { value: SupplierCategory | 'ALL' | 'LUNA_FRIENDS'; label: string; emoji: string }[] = [
    ...
    { value: 'LUNA_FRIENDS', label: 'Luna Friends', emoji: '✨' },
...
export default function PrestationDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await updateCatalogItem(tenantId, item.id, updateData);
            setItem({ ...item, ...updateData } as CRMCatalogItem);
            setEditing(false);
    ...
    return (
        <div className="flex flex-col md:flex-row gap-6 max-w-[1400px] mx-auto p-4 md:p-6 mb-20 animate-in fade-in duration-500">
            ...
            <main className="flex-1 space-y-6">
                ...
                <div className="bg-white rounded-[40px] overflow-hidden border border-gray-100 shadow-sm relative group">
                    <div className={`h-[300px] bg-gradient-to-r ${typeConf.gradient} relative overflow-hidden`}>
                        {item.images && item.images.length > 0 ? (
                            <img src={item.images[0]} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />
                        ) : (
                        ...
                        <div className="absolute bottom-8 left-8 right-8 text-white">
                            ...
                            <h1 className="text-4xl font-normal leading-tight">{item.name}</h1>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-6">
                            ...
                            {linkedSupplier ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="pt-4 border-t border-gray-100 space-y-3">
                                        {linkedSupplier.contactName && <p className="text-xs text-gray-600 flex items-center gap-2">👤 {linkedSupplier.contactName}</p>}
                        ...
                        <div className="bg-luna-charcoal rounded-[32px] p-6 text-white overflow-hidden relative">
                            <Sparkles className="absolute top-4 right-4 text-white/10" size={40} />
                            <h4 className="text-xs font-bold uppercase tracking-widest text-white/50 mb-4">Gestion Expert</h4>
                            <p className="text-sm text-white/90 leading-relaxed font-sans font-light">
                                "Isolez vos coûts et gérez vos bookings en direct sur cette fiche. Le planning est synchronisé avec le prestataire automatique."
                            </p>
                        </div>
                    </div>
                </div>
                                    ...
                                    {(() => {
                                        ...
                                        if (filteredSup.length === 0) return (
                                            <div className="py-12 text-center">
                                                ...
                                                <p className="text-sm text-gray-400 font-sans">Aucun prestataire trouvé</p>
                    ...
                    <div className="bg-white rounded-[40px] w-full max-w-lg relative z-10 shadow-2xl overflow-hidden animate-in slide-in-from-bottom-5">
                        <div className="p-8">
                            ...
                            <div className="space-y-6">
                                ...
                                {bookingSuccess && (
                                    <div className={`p-4 rounded-2xl text-center font-bold text-sm ${bookingSuccess.startsWith('✅') ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'}`}>
                                        {bookingSuccess}
                                    </div>
                                )}
