  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-normal text-luna-charcoal tracking-tight">Activités</h1>
              ...
              <div key={activity.id} className="flex gap-6 items-start group">
                ...
                <div className={`flex-1 bg-white p-5 rounded-2xl border ${isDone ? 'border-luna-warm-gray/15 opacity-60' : 'border-luna-warm-gray/20 shadow-sm'} transition-all hover:shadow-md hover:border-luna-accent/30`}>
                  <div className="flex justify-between items-start mb-2">
                    <h3 className={`font-normal text-lg ${isDone ? 'text-gray-500 line-through' : 'text-gray-900'}`}>{activity.title}</h3>
                    <span className="flex items-center gap-1.5 text-xs font-normal text-gray-400 uppercase tracking-widest bg-gray-50 px-3 py-1 rounded-lg">
                  ...
                  {(activity.contactName || activity.leadId || activity.tripId) && (
                    <div className="flex gap-2 mb-3 flex-wrap">
                      {activity.contactName && (
                        <span className="text-[12px] bg-sky-50 text-sky-600 px-2 py-0.5 rounded-full font-normal flex items-center gap-1">
                          <User size={10} /> {activity.contactName}
                        </span>
                      )}
                      {activity.leadId && (
                        <span className="text-[12px] bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full font-normal flex items-center gap-1">
                          <Target size={10} /> Lead
                        </span>
                      )}
                      {activity.tripId && (
                        <span className="text-[12px] bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full font-normal flex items-center gap-1">
                          <Plane size={10} /> Trip
                        </span>
                      )}
                    </div>
                  ...
                  {!isDone && (
                    <div className="flex gap-3 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleMarkDone(activity.id)} className="text-xs font-normal text-emerald-600 bg-emerald-50 hover:bg-emerald-100 px-4 py-2 rounded-lg transition-colors">Marquer Fait</button>
        ...
        <div className="fixed inset-0 bg-luna-charcoal/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-luxury overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-luna-warm-gray/20 bg-luna-cream">
              <h2 className="font-serif text-xl font-normal text-luna-charcoal">Nouvelle Tâche</h2>
            ...
            <form onSubmit={handleAddTask} className="p-6 space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-normal text-gray-500 uppercase tracking-wider">Titre de l'action</label>
                <input type="text" placeholder="Ex: Appeler Sophie Robert..." required className="w-full px-4 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-luna-accent/30 font-normal text-gray-900" value={newTask.title} onChange={e => setNewTask({ ...newTask, title: e.target.value })} />
              </div>

              {/* Contact selector */}
              <div className="space-y-1">
                <label className="text-xs font-normal text-gray-500 uppercase tracking-wider">Contact lié (optionnel)</label>
                <select className="w-full px-4 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-luna-accent/30 font-normal text-gray-900 appearance-none bg-white"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-normal text-gray-500 uppercase tracking-wider">Date / Heure prévue</label>
                  <input type="text" placeholder="Ex: Demain, 14:00" required className="w-full px-4 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-luna-accent/30 font-normal text-gray-900" value={newTask.time} onChange={e => setNewTask({ ...newTask, time: e.target.value })} />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-normal text-gray-500 uppercase tracking-wider">Type / Urgent</label>
                  <select className="w-full px-4 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-luna-accent/30 font-normal text-gray-900 appearance-none bg-white" value={newTask.type} onChange={e => setNewTask({ ...newTask, type: e.target.value as CRMActivity['type'] })}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-normal text-gray-500 uppercase tracking-wider">Couleur Catégorie</label>
                  <select className="w-full px-4 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-luna-accent/30 font-normal text-gray-900 appearance-none bg-white" value={newTask.color} onChange={e => setNewTask({ ...newTask, color: e.target.value as CRMActivity['color'] })}>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-normal text-gray-500 uppercase tracking-wider">Icône Représentative</label>
                  <select className="w-full px-4 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-luna-accent/30 font-normal text-gray-900 appearance-none bg-white" value={newTask.iconName} onChange={e => setNewTask({ ...newTask, iconName: e.target.value })}>
