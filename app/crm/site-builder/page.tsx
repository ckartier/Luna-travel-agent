...
const LEGAL_DOC_TEMPLATES = [
    ...
    { id: 'mise-en-demeure', name: 'Mise en Demeure', icon: FileSignature, category: 'courrier', color: '#dc2626', description: 'Lettre de mise en demeure formelle pour obtenir l\'exécution d\'une obligation.', content: 'LETTRE RECOMMANDÉE AVEC ACCUSÉ DE RÉCEPTION...' },
...
const TRAVEL_CATEGORIES = [{ id: 'all', label: 'Tous' }, { id: 'auto-reply', label: 'Auto-Reply' }, { id: 'finance', label: 'Finance' }, { id: 'voyage', label: 'Voyage' }, { id: 'portail', label: 'Portail' }];
...
export default function SiteBuilderUnifiedPage() {
    ...
    const handlePreviewDoc = (t: any) => {
        setSelectedDocTemplate(t);
        if (!isLegal && t.previewKey) {
            setPreviewDocUrl(`/api/crm/email-preview?template=${t.previewKey}`);
        }
    };
        ...
        try {
            const res = await fetchWithAuth('/api/gmail/send', {
                ...
                body: JSON.stringify({
                    ...
                    message: `Ceci est un email de test pour le template "${t.name}".`,
                }),
            });
            if (res.ok) {
                setDocSent(true);
    ...
    return (
        <div className="w-full h-full">
            <div className="max-w-[1600px] mx-auto w-full space-y-8 pb-20 p-8">
                ...
                {activeTab === 'web' && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                        <div className="flex justify-between items-end">
                            <div>
                                <h2 className="text-xl font-semibold text-[#2E2E2E]">Templates Web</h2>
                            ...
                            {filteredDocs.map((t, i) => {
                                ...
                                return (
                                    <motion.div key={t.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }}
                                        className="bg-white rounded-2xl border border-gray-100 hover:shadow-lg transition-all cursor-pointer group overflow-hidden"
                                        onClick={() => handlePreviewDoc(t)}>
                                        <div className="h-1" style={{ background: `linear-gradient(90deg, ${t.color}, ${t.color}88)` }} />
                                        <div className="p-6">
                                            <div className="flex justify-between mb-4">
                                                <div className="w-11 h-11 rounded-2xl flex items-center justify-center" style={{ backgroundColor: `${t.color}12` }}>
                                            ...
                                            <h3 className="text-sm font-semibold text-[#2E2E2E] mb-1.5">{t.name}</h3>
            ...
            <AnimatePresence>
                {selectedDocTemplate && activeTab === 'docs' && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        onClick={() => { setSelectedDocTemplate(null); setPreviewDocUrl(null); }}>
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
                            className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl" onClick={e => e.stopPropagation()}>
                            <div className="p-6 border-b flex justify-between" style={{ background: `linear-gradient(135deg, ${selectedDocTemplate.color}08, white)` }}>
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ backgroundColor: `${selectedDocTemplate.color}15` }}>
                                        <selectedDocTemplate.icon size={22} style={{ color: selectedDocTemplate.color }} />
                                    </div>
                                    <div>
                                        <h2 className="text-lg font-semibold text-[#2E2E2E]">{selectedDocTemplate.name}</h2>
                                        <p className="text-xs text-gray-400 mt-0.5">{selectedDocTemplate.description}</p>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                    {!isLegal && (
                                        <button onClick={() => handleSendTestDoc(selectedDocTemplate)} disabled={docSending}
                                            className="flex gap-2 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest text-white transition-all disabled:opacity-50"
                                            style={{ backgroundColor: accentColor }}>
                                            {docSending ? <Loader2 size={14} className="animate-spin" /> : docSent ? <CheckCircle2 size={14} /> : <Send size={14} />}
                                            {docSending ? 'Envoi...' : docSent ? 'Envoyé' : 'Test Email'}
                                        </button>
                                    )}
                                    {isLegal && (
                                        <button onClick={() => handleCopyDoc(selectedDocTemplate.content)}
                                            className="flex gap-2 px-4 py-2 rounded-xl text-xs font-bold uppercase text-white transition-all"
                                            style={{ backgroundColor: accentColor }}>
                                            {docCopied ? <CheckCircle2 size={14} /> : <Copy size={14} />}
                                            {docCopied ? 'Copié' : 'Copier'}
                                        </button>
                                    )}
                                    <button onClick={() => { setSelectedDocTemplate(null); setPreviewDocUrl(null); }} className="p-2 text-gray-400 hover:bg-gray-100 rounded-xl">
                                        <X size={18} />
                                    </button>
                                </div>
                            </div>
                            <div className="flex-1 overflow-y-auto">
                                {isLegal ? (
                                    <div className="p-8"><pre className="text-sm text-gray-700 bg-gray-50 p-8 rounded-2xl whitespace-pre-wrap font-sans">{selectedDocTemplate.content}</pre></div>
                                ) : previewDocUrl ? (
                                    <iframe src={previewDocUrl} className="w-full h-[70vh] border-0" />
                                ) : (
                                    <div className="flex justify-center h-[400px] text-gray-300 items-center"><Loader2 className="animate-spin" size={32} /></div>
                                )}
                            </div>
                        </motion.div>
                    </motion.div>
