  ...
  return new Promise(async (resolve) => {
    try {
      ...
      session.client.on('close', (event) => {
         console.error(`❌ FAILED: ${tool.name}. Code: ${event.code}, Reason: ${event.reason}`);
         resolve(false);
      });
      
      session.client.on('open', () => {
         console.log(`✅ SUCCESS: ${tool.name}`);
         setTimeout(() => { session.close(); resolve(true); }, 500);
      });

    } catch (err) {
      console.error(`❌ EXCEPTION for ${tool.name}:`, err.message || err);
      resolve(false);
    }
  });
}
...
async function run() {
  console.log(`Testing ${TRAVEL_TOOLS.length} tools individually...`);
